...
period_spac_total = []
period_hvsr_total = []  # format: dict in list
period_name = []
period_init = 1
...
for i in range(period_num - 1):
    point_same = period_spac_total[i].keys() & period_spac_total[i + 1].keys()  # format: set
...
for point_name in point_same:
    ...
    if is_ring_same:
        ...
        pass
    # hvsr
    hvsr_all = []  # format: dataframe in list
