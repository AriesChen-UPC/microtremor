...
if fileType == 'Y':
    ...
    # for i in range(len(fs)):
        ...
        # spac.append(targetInstance.AutocorrCurves.ModalCurve[numRings].RealStatisticalPoint['mean'][-freqLen-1:-1])
        # freq.append(targetInstance.AutocorrCurves.ModalCurve[numRings].RealStatisticalPoint['x'][-freqLen-1:-1])
    ...
    for i in range(len(fs)):
        # print(fs[i])
        spac.append(read_page(fs[i])[0][numRings]['spac'][-freqLen-1:-1])  # fixme: data dimension is not the same
        freq.append(read_page(fs[i])[0][numRings]['freq'][-freqLen-1:-1])  # fixme: delete the nan data in list
        # spac.append([0 if math.isnan(x) else x for x in read_page(fs[i])[0][numRings]['spac']])
        # freq.append([0 if math.isnan(x) else x for x in read_page(fs[i])[0][numRings]['freq']])
