...
class AutocorrTarget:
    ...
    def __init__(self, file=None, verb=True):
    ...
    def load(self, file, cleanup=True, verbose=True):
        if cleanup:
            self.__init__()
    ...
    def write(self, filename='/Users/tenqei/Desktop/test_xml.target', verbose=True):
    ...
    def fromSPAC(self, freq: np.ndarray, spac: np.ndarray, std: np.ndarray, rings=None, weights=None):
        ...
        if weights is None:
            weights = np.ones(shape=spac.shape)
        ...
        for iring in range(len(rings)):
            ...
            tmp0.RealStatisticalPoint['weight'] = weights[iring]
...
class SignalDatabase:
    ...
    def write(self, filename='/Users/tenqei/Desktop/test_xml.gpy', verbose=True):
        # create new root element
        doc = minidom.Document()
...
class HVfile:
    ...
    def __init__(self, hv: Hvsr or list = None, distribution_mc='log-normal', distribution_f0='log-normal', pos=None,
                 fromfile=None):
        if fromfile:
            self.read(fromfile)
        else:
            ...
            self.meta['position'] = pos
            self.meta['category'] = 'Default'

    def write(self, filename=None, filepath='/Users/tenqei/Desktop', verbose=True):
