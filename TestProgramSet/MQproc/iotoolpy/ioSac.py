...
def read_sac(source, headonly=False, byteorder=None, checksize=False):
    ...
    """
    # TODO: rewrite using "with" statement instead of open/close utils.
...
def validate_sac_content(hf, hi, hs, data, *tests):
    ...
    if 'reftime' in tests:
        ...
        try:
            datetime.strptime('.'.join([str(x) for x in [nzyear, nzyear, nzhour, nzmin, nzsec, nzmsec]]),
                              '%Y.%j.%H.%M.%S.%f')
