...
class HvsrRotated:
    ...
    @property
    def amp(self):
        if self.hvsrs[0].n_windows == 1:
            return [hv.amp for hv in self.hvsrs]
        else:
    ...
    def _stats(self, distribution_f0):
        ...
        df = dict(data=data, columns=columns,
                  index=["Fundamental Site Frequency, f0,AZ", "Fundamental Site Period, T0,AZ"])
        return df
