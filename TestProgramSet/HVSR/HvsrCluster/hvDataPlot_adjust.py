...
fig.update_yaxes(range=[0.0, 3.0], tick0=0.0, dtick=0.5)
fig.update_layout(title=file_path.split("/")[-1].split(".")[-2], showlegend=False)
fig.update_xaxes(title_text="Frequency (Hz)")
fig.update_yaxes(title_text="Amplitude")
# extract filepath and filename
filepath, tempfilename = os.path.split(file_path)
# make new filepath for storing .htlm file
dirs = filepath + '/' + 'html'
if not os.path.exists(dirs):
    os.makedirs(dirs)
# name the .html file  # todo:filename splited by '.', renew the extract style
plotly.offline.plot(fig, filename=dirs + '/' + file_path.split("/")[-1] + '.html')
