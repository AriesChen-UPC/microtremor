...
for model in model_list:
    print("\033[0;36mThe current model is: %s.\033[0m" % model)
    time_start = time.time()
    V = Variogram(data[['x', 'y']].values, data.vs.values, model=model, bin_func='kmeans', maxlag='median')
    ...
    fig_font_size = max(int(x_grid_num/10), int(y_grid_num/10))+5
    ...
    art = axes.matshow(field.T, origin='lower', cmap='RdYlBu_r', vmin=vs.min(), vmax=vs.max())  # _r means reverse
    axes.set_title(model, fontsize=fig_font_size, fontweight='bold', pad=25)
    ...
    plt.show()
    time_end = time.time()
    print("\033[0;33mThe time consumed by %s model is %.2f seconds. \033[0m" % (model, time_end - time_start))
time_end_main = time.time()
print("\033[0;31mThe time consumed by all models is %.2f seconds. \033[0m" % (time_end_main - time_start_main))
