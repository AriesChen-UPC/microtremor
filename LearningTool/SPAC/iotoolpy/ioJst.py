"""Depreciated functions to read JST files"""
...
from iotoolpy.ioSac import dict_to_header_arrays, write_sac
...
def read_JST_int0(file, sta='', net='', chn='BHZ'):
    ...
    if not chk:
        ...
        st = datetime(1980, 1, 6) + timedelta(t0 / sampling_rate)
        dt = 1 / sampling_rate
        n = int(os.path.getsize(file) / 4)
        stat0 = {'npts': n, 'delta': dt, 'b': 0, 'e': (n - 1) * dt, 'st': st,
                 'kstnm': sta, 'knetwk': net, 'kcmpnm': chn,
                 'nzyear': st.year, 'nzjday': int(st.strftime('%j')), 'nzhour': st.hour, 'nzmin': st.minute,
