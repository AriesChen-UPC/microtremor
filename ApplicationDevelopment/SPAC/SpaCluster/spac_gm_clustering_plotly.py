...
def spac_gm_clustering_plotly(folder_path, fs, freq, spac, names, freq_theory_spac, spac_theory_spac, min_freq, max_freq,
    ...
    if gm_components == 1:
        ...
        numRow = 2
        numCol = int(np.ceil(gm_components / 2))
        fig = make_subplots(rows=numRow, cols=numCol)
        for z in range(gm_components):  # z is the number of classification
            if z < numCol:
                fig.add_trace(go.Scatter(x=freq_theory_spac['freq'], y=spac_theory_spac['autoCorrRatio'], name='theory SPAC',
                                         mode='lines', showlegend=False,
                                         line=dict(
        ...
        for z in range(gm_components):
            for k in range(len(fs)):
                if gm_clustering[k] == z:
                    if z < numCol:
                        fig.add_trace(go.Scatter(x=freq[k], y=spac[k], name=names[k], showlegend=True),
                                      row=1, col=z+1)
                    else:
    ...
    default_html_name = folder_path + '/' + 'spac_gm_cluster.html'
