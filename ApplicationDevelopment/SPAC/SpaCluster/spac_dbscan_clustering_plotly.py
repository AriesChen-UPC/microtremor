...
def spac_dbscan_clustering_plotly(folder_path, fs, freq, spac, names, freq_theory_spac, spac_theory_spac, min_freq, max_freq,
                           radius, vs_reference, dbscan_clustering):
    """
    Args:
    ...
    for k in range(len(fs)):
        if dbscan_clustering.labels_[k] == -1:
    ...
    for i in range(len(fs)):
        if dbscan_clustering.labels_[i] == -1:
    ...
    default_html_name = folder_path + '/' + 'spac_dbscan_cluster.html'
