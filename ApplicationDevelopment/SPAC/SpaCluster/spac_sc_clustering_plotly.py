...
def spac_sc_clustering_plotly(folder_path, fs, freq, spac, names, freq_theory_spac, spac_theory_spac, min_freq, max_freq,
                              radius, vs_reference, sc_clustering, sc_clusters):
    """
    Args:
        ...
        sc_clusters: the number of clusters.
    ...
    if sc_clusters == 1:
        ...
        for k in range(len(fs)):
            if sc_clustering.labels_[k] == 0:
                fig.add_trace(go.Scatter(x=freq[k], y=spac[k], name=names[k], showlegend=True),
                              row=1, col=1)
            elif sc_clustering.labels_[k] == 1:
                fig.add_trace(go.Scatter(x=freq[k], y=spac[k], name=names[k], showlegend=True),
                              row=1, col=2)
            else:
                fig.add_trace(go.Scatter(x=freq[k], y=spac[k], name=names[k], showlegend=True),
        ...
        numRow = 2
        numCol = int(np.ceil(sc_clusters / 2))
        fig = make_subplots(rows=numRow, cols=numCol)
        for z in range(sc_clusters):  # z is the number of classification
            if z < numCol:
                fig.add_trace(go.Scatter(x=freq_theory_spac['freq'], y=spac_theory_spac['autoCorrRatio'], name='theory SPAC',
                                         mode='lines', showlegend=False,
                                         line=dict(
        ...
        for z in range(sc_clusters):
            for k in range(len(fs)):
                if sc_clustering.labels_[k] == z:
                    if z < numCol:
                        fig.add_trace(go.Scatter(x=freq[k], y=spac[k], name=names[k], showlegend=True),
                                      row=1, col=z+1)
                    else:
    ...
    default_html_name = folder_path + '/' + 'spac_sc_cluster.html'
