...
class Hvsr:
    ...
    def __init__(self, amplitude, frequency, find_peaks=True, meta=None):  # TODO: issue with find_peaks, temp. removed
        ...
        ----------
        amplitude : ndarray
            Array of H/V amplitudes. Each row represents an individual curve and each column a frequency.
        frequency : ndarray
            Vector of frequencies, corresponding to each column.
        find_peaks : bool, optional
            Indicates whether peaks of Hvsr will be found when created, default is `True`.
        meta : dict, optional
        ...
        self.valid_frequencies = np.where((self.frq >= 0.3) * (self.frq <= 30))[0]
    ...
    def update_peaks(self, **kwargs):
        ...
        **kwargs : dict
        ...
        if self.n_windows == 1:
            if self.valid_frequencies.shape == self.frq.shape:
                peak_indices, _ = self.find_peaks(self.amp, **kwargs)
            else:
                peak_indices = self.find_peaks(self.amp[self.valid_frequencies], **kwargs)[0] \
                               + self.valid_frequencies[0]
        ...
        if self.valid_frequencies.shape == self.frq.shape:
            peak_indices, _ = self.find_peaks(self.amp[self.valid_window_indices], **kwargs)
        else:
            p0, _ = self.find_peaks(self.amp[self.valid_window_indices][:, self.valid_frequencies], **kwargs)
            peak_indices = [ip0 + self.valid_frequencies[0] for ip0 in p0]
        ...
        for c_window, c_window_peaks in zip(self.valid_window_indices, peak_indices):
            try:
                ...
                if len(c_index[0]) > 1:
                    c_index = (c_index[0][0])  # TODO: temporarily solved peaks more than 1 issue, need verification
    ...
    def std_curve(self, distribution='log-normal'):
        ...
        if self.n_windows == 1:
            ...
            return np.zeros(self.frq.shape)
        else:
    ...
    def mc_peak_frq(self, distribution='log-normal'):
        ...
        ifrq = [x for x in self.find_peaks(mc)[0] if x in self.valid_frequencies]
        return float(self.frq[np.where(mc == np.max(mc[ifrq]))])
    ...
    def reject_windows(self, n=2, max_iterations=50,
        ...
        for c_iteration in range(1, max_iterations + 1):
            ...
            for c_window, c_peak in zip(self.valid_window_indices, self.peak_frq):
                if lower_bound < c_peak < upper_bound:
    ...
    def _stats(self, distribution_f0):
        ...
        df = dict(data=data, columns=columns, index=["Fundamental Site Frequency, f0", "Fundamental Site Period, T0"])
        return df
